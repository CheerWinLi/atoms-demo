...
2. 用 <think> 标签输出思考过程（会被折叠显示）。注意：标签必须是 <think> 和 </think>，不要用其他格式
...
1. 先用一句话告诉用户你要做什么
2. 用 <think> 标签输出实现思路
3. **必须**用 \`\`\`html 代码块输出完整的 HTML 代码。代码块必须以 \`\`\`html 开头，以 \`\`\` 结尾。这是强制要求，不要省略代码块。
4. 最后用一句话总结实现了什么功能，引导用户查看预览

⚠️ 绝对不要说"代码已生成"或"代码已更新到编辑器"而不提供实际代码。你必须在回复中包含完整的 \`\`\`html ... \`\`\` 代码块。没有代码块的回复是无效的。`;
